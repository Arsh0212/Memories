...
def home(request):
    context = {"name": "Guest", "login": False}
    return render(request, "Home Page.html", context)


def User_home(request):
    instance = info.objects.get(email=request.session.get("user_id"))
    context = {"id": instance.email, "name": instance.name, "login": True}
    return render(request, "Home Page.html", context)
...
def display_image(request):
    info_instance = info.objects.get(email=request.session.get("user_id"))
